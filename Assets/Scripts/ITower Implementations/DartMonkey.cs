...
public class DartMonkey : MonoBehaviour, ITower
{
    public TowerType TowerType => TowerType.DartMonkey;
    public ItemType UsedItem => ItemType.DartMonkeyProjectile;
    ...
    public IEnumerator Behave()
    {
        while (true)
        {
            ...
            {
                Helper.LookAtTarget(gameObject, BloonTarget);

                GameObject projectile = ItemsPoolsManager.Instance.GetItem(UsedItem);
                projectile.transform.position = transform.position;

                IItem iItem = projectile.GetComponent<IItem>();
                iItem.SetNewOwner(gameObject);
                iItem.SetNewTarget(BloonTarget);
                iItem.PerformAction();
